...
public class FailoverOperatorTest {
  ...
  public void testFailover() throws HectorException,
    ...
    doThrow(new org.apache.cassandra.thrift.TimedOutException()).when(h1cassandra).insert(
        anyString(), anyString(), (ColumnPath) anyObject(), (byte[]) anyObject(), anyLong(),
        Matchers.<ConsistencyLevel> any());
    try {
      ks.insert("key", cp, bytes("value"));
