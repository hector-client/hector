...
/**
 * A spring factory bean that is used for injecting templates. It makes a proxy so that additional
 * functionalities, like multi-method batches can be supported
 *
...
public class HectorTemplateFactoryBean implements FactoryBean<HectorTemplate> {
  ...
  public HectorTemplate getObject() throws Exception {
    ...
    return (HectorTemplate) Proxy.newProxyInstance(getClass().getClassLoader(),
        new Class[] { HectorTemplate.class }, handler);
  }
