 ...
 */
public final class SubColumnQuery<K,SN,N,V> implements Query<HColumn<N,V>> {

  private final SubSliceQuery<K,SN,N,V> subSliceQuery;

  /*package*/ public SubColumnQuery(KeyspaceOperator keyspaceOperator, Serializer<K> keySerializer,
      Serializer<SN> sNameSerializer, Serializer<N> nameSerializer, Serializer<V> valueSerializer) {
    subSliceQuery = HFactory.createSubSliceQuery(keyspaceOperator, keySerializer, sNameSerializer,
