...
public class Rows<N, V> implements Iterable<Row<N, V>> {
  ...
  protected final Map<byte[], Row<N, V>> rows;

  public Rows(Map<byte[], List<Column>> thriftRet, Extractor<N> nameExtractor,
      Extractor<V> valueExtractor) {
    Assert.noneNull(thriftRet, nameExtractor, valueExtractor);
    rows = new HashMap<byte[], Row<N, V>>(thriftRet.size());
  ...
  public Row<N, V> getByKey(byte[] key) {
    return rows.get(key);
  }
