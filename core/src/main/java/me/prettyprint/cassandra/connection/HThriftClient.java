...
public class HThriftClient {
	...
	if (keyspaceNameArg != null
		&& !StringUtils.equals(keyspaceName, keyspaceNameArg)) {
	    if (log.isDebugEnabled())
		log.debug("keyspace reseting from {} to {}", keyspaceName,
			keyspaceNameArg);
	...
	if (isOpen()) {
		...
		log.error(
			"Could not flush transport (to be expected if the pool is shutting down) in close for client: "
				+ toString(), e);
	    } finally {
		try {
		    transport.close();
		} catch (Exception e) {
		    log.error("Error on transport close for client: "
			    + toString(), e);
		}
	    }
	}
	...
	TSocket socket = new TSocket(cassandraHost.getHost(),
		cassandraHost.getPort(), timeout);
	if (cassandraHost.getUseSocketKeepalive()) {
	    try {
		socket.getSocket().setKeepAlive(true);
	    } catch (SocketException se) {
		throw new HectorTransportException(
	...
	try {
	    ...
	    throw new HectorTransportException(
		    "Unable to open transport to " + cassandraHost.getName()
    ...
    /**
     * If CassandraHost was not null we use
     * {@link CassandraHost#getCassandraThriftSocketTimeout()} if it was greater
     * than zero. Otherwise look for an environment variable name
     * CASSANDRA_THRIFT_SOCKET_TIMEOUT value. If doesn't exist, returns 0.
     * 
     * @param cassandraHost
     */
	...
	if (cassandraHost != null
		&& cassandraHost.getCassandraThriftSocketTimeout() > 0) {
	    timeoutVar = cassandraHost.getCassandraThriftSocketTimeout();
	} else {
	    String timeoutStr = System
		    .getProperty(SystemProperties.CASSANDRA_THRIFT_SOCKET_TIMEOUT
			    .toString());
		...
		try {
		    ...
		    log.error(
			    "Invalid value for CASSANDRA_THRIFT_SOCKET_TIMEOUT",
			    e);
		}
	    }
	}
