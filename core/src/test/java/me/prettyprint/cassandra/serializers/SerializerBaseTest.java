...
/**
 * A base test class for {@link Serializer}s. Tests for:
 * 1. Valid serialization roundtrip for a collection of objects.
 * 2. Proper handling of NULL.
 * 
 * @author shuzhang0@gmail.com
 *
 * @param <T> the type which the Serializer under test can serialize.
 */
public abstract class SerializerBaseTest<T> {

    /** Validate serialization roundtrip for a collection of objects (defined by subclasses). */
    @Test
    public void testRoundTrip() {
        for (T object : getTestData()) {
            Assert.assertEquals(object, getSerializer().fromByteBuffer(getSerializer().toByteBuffer(object)));
        }
    }
    ...
    public void testNullObjectRoundTrip() {
        Assert.assertNull(getSerializer().fromByteBuffer(getSerializer().toByteBuffer(null)));
    }
